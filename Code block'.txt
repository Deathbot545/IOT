 #define BLYNK_TEMPLATE_ID "TMPL6RzBuqbnW"
...
#define RELAY_PIN 4             // Relay for  Pump  out Control
#define RELAY_PIN_Pump 5        // Relay for  Water Pump IN Control

// WiFi & Blynk credentials
char auth[] = "LwgDeKUWhN52Ix_ZDcziHPvHNcyV_rwK";   // Y9_NPbl2aXYw0oeZ47NEqZ7Yr9HPXi5p
char ssid[] = "OPPOF17";    
char pass[] = "12345678";  
...
BLYNK_WRITE(V1) {
  ...
  if (switchState == 1) {
    isAutoControl = true;
    servoTimer1 = timer.setInterval(8000L, []() { // Total cycle: 3s ON + 5s OFF // 5 seconds for imagine like 12 hours
...
void checkSystem() {
  ...
  // Automatic water remove  Logic (Only if Manual Mode is OFF)
  if (!isManualPump) {
    if (tempC > 35.0) {
   ...
   // Automatic Pump IN Logic (Only if Manual Mode is OFF)
  if (!isManualPump) {
    if (tempC > 35.0) {
      Serial.println("Temperature high! Pump ON.");
      digitalWrite(RELAY_PIN_Pump, HIGH);
